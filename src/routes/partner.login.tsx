...
export const Route = createFileRoute("/partner/login")({
  head: () => ({
    meta: [
      { title: "Partner Login — Natural State Peptides" },
...
function Login() {
  ...
  async function login(e: React.FormEvent) {
    ...
    else await navigate({ to: "/partner/dashboard" });
  }
  async function reset() {
    ...
    const { error } = await db.auth.resetPasswordForEmail(email, {
      redirectTo: (publicSite || window.location.origin) + "/partner/reset",
    });
    setStatus(
  ...
  return (
    <section className="mx-auto max-w-md px-5 py-20">
      ...
      <div className={panel}>
        {!backendReady ? (
          <p className="text-sm">
            Partner access is not open yet.{" "}
      ...
      <p className="mt-6 text-sm text-muted-foreground">
        Accounts are issued to approved partners.{" "}
        <Link to="/ambassador" className="underline">
          Apply to join.
        </Link>
      </p>
