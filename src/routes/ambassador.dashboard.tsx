  ...
  if (!user)
    return (
      <div className="mx-auto max-w-lg px-5 py-20">
        <h1 className="font-serif text-4xl">Your ambassador account</h1>
  ...
  if (!data)
    return (
      <div className="mx-auto max-w-xl px-5 py-20">
        <h1 className="font-serif text-4xl">{admin ? "Owner account" : "Ambassador access"}</h1>
        <p className="my-5" role="status">
          {admin
            ? "Open owner controls to manage ambassadors and recorded sales."
            : status || "Loading your records…"}
        </p>
        {admin && (
          <Link to="/admin/ambassadors" className={button}>
            Open owner controls
          </Link>
        )}
    ...
    <section className="mx-auto max-w-7xl px-5 py-12 sm:px-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          {admin && (
            <Link to="/admin/ambassadors" className={outline}>
              Owner controls
            </Link>
          )}
